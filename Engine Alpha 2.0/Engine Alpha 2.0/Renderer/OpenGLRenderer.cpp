...
OpenGLRenderer* OpenGLRenderer::create(int argc, char* argv[], const char* display_name)
{
	...
	if (_pInstance && _pInstance->init(argc, argv) && _pInstance->initDisplay(display_name) && _pInstance->initOpenGLExtensions())
	{
		...
		ProgramManager* programmanager = ProgramManager::create();
		DrawingManager* drawingmanager = DrawingManager::create();
		BufferManager* buffermanager = BufferManager::create();

		assert(shadermanager && programmanager && drawingmanager && buffermanager);

		_pInstance->setShaderManager(shadermanager);
		_pInstance->setProgramManager(programmanager);
		_pInstance->setDrawingManager(drawingmanager);
		_pInstance->setBufferManager(buffermanager);

		LuaController::getInstance()->setOpenGLRenderer(_pInstance);
	}
	else
	{
...
{
	pushUniform("model_matrix", _model_matrix);
	pushUniform("view_matrix", _view_matrix);
	pushUniform("projection_matrix", _projection_matrix);
