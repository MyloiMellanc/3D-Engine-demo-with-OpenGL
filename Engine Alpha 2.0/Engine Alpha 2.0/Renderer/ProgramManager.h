...
public:
	...
	*/
	Program* getProgram(const char* program_name);
	Program* getProgram(const GLuint program_id);

	/*
	* getter of program pipeline pointer.
	* using program name, or ID.
	* if fail to search, it return nullptr.
	*/
	ProgramPipeline* getProgramPipeline(const char* pipeline_name);
	ProgramPipeline* getProgramPipeline(const GLuint program_id);
